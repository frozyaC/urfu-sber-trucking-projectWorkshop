...
export function LogisticianDashboard({ 
  ...
  const [lastApiLoad, setLastApiLoad] = useState<{ time: Date; added: number; duplicates: number } | undefined>();
  ...
  /**
   * Загрузка водителей из внешнего API при нажатии "Добавить заказ"
   * 
   * Процесс:
   * 1. Загрузка данных из MockAPI
   * 2. Проверка на дубликаты по номеру ВУ
   * 3. Добавление новых водителей
   * 4. Показ уведомления о результате
   * 5. Открытие управления автопарком или модального окна заказа
   */
  const handleAddOrderClick = async () => {
    setIsLoadingDrivers(true);
    
    try {
      // Загружаем водителей из API через сервис
      const result = await loadDriversFromApi();
      
      if (!result.success) {
        toast.error(result.error || 'Не удалось загрузить водителей из API. Проверьте подключение к интернету.');
        setIsAddOrderModalOpen(true);
        return;
      }
      
      const apiDrivers = result.drivers;
      
      // Счетчики для статистики
      let addedCount = 0;
      let duplicateCount = 0;
      
      // Обрабатываем каждого водителя из API
      apiDrivers.forEach((apiDriver) => {
        // Проверяем, не существует ли уже водитель с таким номером ВУ
        const existingDriver = drivers.find(d => d.licenseNumber === apiDriver.licenseNumber);
        ...
        if (!existingDriver) {
          ...
          onAddDriver({
            ...
            availability: apiDriver.availability
          });
          addedCount++;
        } else {
          duplicateCount++;
      ...
      // Показываем уведомления о результате
      if (addedCount > 0) {
        if (duplicateCount > 0) {
          toast.success(`Загружено водителей: ${addedCount} (пропущено дубликатов: ${duplicateCount})`);
        } else {
          toast.success(`Загружено водителей: ${addedCount}`);
        }
        // Открываем управление автопарком на вкладке "Водители"
        setIsFleetManagementOpen(true);
      } else if (duplicateCount > 0) {
        toast.warning(`Все водители (${duplicateCount}) уже существуют в системе`);
        // Все равно открываем управление автопарком, чтобы показать существующих водителей
        setIsFleetManagementOpen(true);
      } else if (apiDrivers.length === 0) {
        toast.warning('Не найдено водителей для загрузки');
        // Открываем модальное окно добавления заказа
        setIsAddOrderModalOpen(true);
      }
      
      console.log(`📊 Статистика загрузки водителей из API:`, {
        загружено: addedCount,
        дубликатов: duplicateCount,
        всего_получено: apiDrivers.length
      });
    } catch (error) {
      console.error('❌ Ошибка при загрузке водителей:', error);
      toast.error('Не удалось загрузить водителей из API. Проверьте подключение к интернету.');
      // В случае ошибки все равно открываем модальное окно добавления заказа
      setIsAddOrderModalOpen(true);
    } finally {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {isLoadingDrivers ? 'Загрузка...' : 'Добавить заказ'}
              </Button>
              <Button 
